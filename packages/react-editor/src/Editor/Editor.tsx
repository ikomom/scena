...
export default class Editor extends React.PureComponent<{
  ...
  public async componentDidMount() {
    ...
    this.keyManager.keydown([isMacintosh ? "meta" : "ctrl", "x"], () => {
    }, "Cut");
    this.keyManager.keydown([isMacintosh ? "meta" : "ctrl", "c"], () => {
    }, "Copy");
    // this.keyManager.keydown([isMacintosh ? "meta" : "ctrl", "shift", "c"], e => {
    //     this.clipboardManager.copyImage();
    //     e.inputEvent.preventDefault();
    // }, "Copy to Image");
    this.keyManager.keydown([isMacintosh ? "meta" : "ctrl", "v"], () => {
    }, "Paste");
  ...
  public removeElements(targets: Array<HTMLElement | SVGElement>, isRestore?: boolean) {
    ...
    return viewport.removeTargets(targets).then(({removed}) => {
      const selectedTarget = selectedInfo || viewport.getLastChildInfo(scopeId)! || viewport.getInfo(scopeId);
